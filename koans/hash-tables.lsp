...
(define-test test-create-hash-table
  ...
  (let ((my-hash-table))
    ...
    (true-or-false? ()  (hash-table-p (make-array '(3 3 3))))
...
(define-test test-hash-table-access
  ...
  (assert-equal nil (gethash -3 table-of-cube-roots)) ; tricky bastards
...
(define-test test-hash-table-equality
    (let ((h1 (make-hash-table :test #'equal))
      ...
      (true-or-false? () (eq h1 h2))
      (true-or-false? () (equal h1 h2))
...
(define-test test-make-your-own-hash-table
  ...
  (let ((colors (make-hash-table :test #'equal))
      ...
      (setf (gethash "orange" colors) '(0 0 0))
